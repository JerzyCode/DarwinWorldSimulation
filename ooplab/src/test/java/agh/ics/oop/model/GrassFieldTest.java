...
class GrassFieldTest {
    ...
    @Test
    void shouldSortAnimals() {
        //given
        var grassField = new GrassField(100);
        var shouldFirst = new Animal(new Vector2d(0, 1));
        var shouldSecond = new Animal(new Vector2d(0, 3));
        var shouldThird = new Animal(new Vector2d(4, 1));

        try {
            grassField.place(shouldThird);
            grassField.place(shouldSecond);
            grassField.place(shouldFirst);
        } catch (IncorrectPositionException e) {
            fail("Test animal should be added to map!");
        }

        //when
        var sortedAnimals = (List<Animal>)grassField.getOrderedAnimals();

        //then
        assertEquals(3, sortedAnimals.size());
        assertEquals(shouldFirst, sortedAnimals.getFirst());
        assertEquals(shouldSecond, sortedAnimals.get(1));
        assertEquals(shouldThird, sortedAnimals.getLast());
